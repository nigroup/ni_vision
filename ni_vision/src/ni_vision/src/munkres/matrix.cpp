...
/*export*/ template <class T>
MunkresMatrix<T>::MunkresMatrix(const MunkresMatrix<T> &other) {
...
/*export*/ template <class T>
MunkresMatrix<T>::MunkresMatrix(int rows, int columns) {
    m_matrix = NULL;
    resize(rows, columns);
}
...
/*export*/ template <class T>
void
MunkresMatrix<T>::resize(int rows, int columns) {
    if ( m_matrix == NULL ) {
        // alloc arrays
...
MunkresMatrix<T>::product(MunkresMatrix<T> &other) {
    ...
    MunkresMatrix<T> out(m_rows, other.m_columns);
...
/*export*/ template <class T>
inline T&
MunkresMatrix<T>::operator ()(int x, int y) {
    assert ( x >= 0 );
    assert ( y >= 0 );
    assert ( x < m_rows );
    assert ( y < m_columns );
